...
from lnhistoryclient.parser.common import strip_known_message_type, decode_alias
from lnhistoryclient.model.ChannelUpdate import ChannelUpdate
from lnhistoryclient.model.NodeAnnouncement import NodeAnnouncement, Address
...
from lnhistoryclient.constants import MSG_TYPE_NODE_ANNOUNCEMENT, MSG_TYPE_CHANNEL_ANNOUNCEMENT, MSG_TYPE_CHANNEL_UPDATE

# --- CONFIGURATION ---
LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO")
POSTGRES_URI = os.getenv("POSTGRES_URI")
ZMQ_SOURCES = os.getenv("ZMQ_SOURCES", "tcp://host.docker.internal:5675,tcp://host.docker.internal:5676").split(",")
BATCH_SIZE = int(os.getenv("BATCH_SIZE", "200"))     # Process 200 messages per transaction
BATCH_TIMEOUT = float(os.getenv("BATCH_TIMEOUT", "1.0")) # Or wait 1 second
...
ORPHANS = Counter('gossip_orphans_total', 'Updates for unknown channels')
...
class Database:
    ...
    def _handle_channel_update(self, cur, gossip_id, dt, raw_bytes, data: ChannelUpdate):
        ...
        if cur.fetchone() is None:
            ORPHANS.inc()
            # We continue anyway to store the data
...
class GossipProcessor:
    ...
    def zmq_worker(self, zmq_uri):
        ...
        sock.setsockopt(zmq.RCVBUF, 1024 * 1024) # Increase buffer
        try:
    ...
    def db_worker(self):
        logger.info("--- DB Worker started (Batch Mode) ---")
        while self.running:
            ...
            # Try to grab more from queue without waiting, up to BATCH_SIZE
            start_time = time.time()
            while len(batch) < BATCH_SIZE:
                try:
                    # Small timeout to allow batch accumulation
